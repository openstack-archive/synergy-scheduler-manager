...
class NovaBaseAPI(BaseAPI):
    def __init__(self, topic):                
        ...
        self.target = messaging.Target(topic=topic, namespace="baseapi", version="1.0")
                         
        LOG.info("creating RPC client for topic '%s'" % (topic + "_synergy"))
        self.client = rpc.getClient(target=messaging.Target(topic=topic + "_synergy", namespace="baseapi", version="1.0"))
...
class NovaConductorAPI(ConductorAPI):
    ...
    def __init__(self, topic):        
        report_interval = cfg.IntOpt('report_interval', default=10, help='Seconds between nodes reporting state to datastore')        
        cfg.CONF.register_opt(report_interval)
        
        LOG.info("creating RPC server for topic '%s'" % topic)
        self.target = messaging.Target(topic=topic, namespace=None, version="2.0")
                         
        LOG.info("creating RPC client for topic '%s'" % (topic + "_synergy"))
        self.client = rpc.getClient(target=messaging.Target(topic=topic + "_synergy", version="2.0"), version_cap="2.0")
            
            
    def instance_get_by_uuid(self, context, instance_uuid, columns_to_join=None):
        #LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> instance_get_by_uuid instance_uuid=%s" % instance_uuid)
        cctxt = self.client.prepare()
        return cctxt.call(context, 'instance_get_by_uuid', instance_uuid=instance_uuid, columns_to_join=columns_to_join)


    def service_get_all_by(self, context, topic=None, host=None, binary=None):
        #LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> service_get_all_by topic=%s" % topic)
        cctxt = self.client.prepare()
        #cctxt = self.client.prepare(version_cap="1.9")
        return cctxt.call(context, 'service_get_all_by', topic=topic, host=host, binary=binary)

    #???????????????????
    def service_create(self, context, values):
        #LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> service_create values=%s" % values)
        cctxt = self.client.prepare()
    ...
    def object_class_action(self, context, objname, objmethod, objver, args, kwargs):
        #LOG.info("LLLLLLLLLLLLLLLLLLL object_class_action")
        cctxt = self.client.prepare()
        return cctxt.call(context, 'object_class_action', objname=objname, objmethod=objmethod, objver=objver, args=args, kwargs=kwargs)
...
class NovaConductorAPI_FIX(ConductorAPI):
    ...
    def __init__(self, topic):
        ...
        LOG.info("creating RPC server for topic '%s'" % topic)
        self.target = messaging.Target(topic=topic, namespace=None, version="1.9")
    ...
    def object_class_action(self, context, objname, objmethod, objver, args, kwargs):
        #LOG.info("ZZZZZZZZZZZZZZZZZZz object_class_action")
        cctxt = self.client.prepare()
        return cctxt.call(context, 'object_class_action', objname=objname, objmethod=objmethod, objver=objver, args=args, kwargs=kwargs)
...
class NovaConductorComputeAPI(ComputeTaskAPI):
    ...
    def __init__(self, topic, processRequest, keystone_client):
        ...
        LOG.info("creating RPC server for topic '%s'" % topic)
        self.target = messaging.Target(topic=topic, namespace='compute_task', version="1.9")
        
        LOG.info("creating RPC client for topic '%s'" % (topic + "_synergy"))
        self.client = rpc.getClient(target=messaging.Target(topic=topic + "_synergy", namespace='compute_task', version="1.9"))
    ...
    def build_instances(self, context, instances, image, filter_properties, admin_password, injected_files, requested_networks, security_groups, block_device_mapping=None, legacy_bdm=True):
        #LOG.info(">>>>>>>>>>>>>>>  build_instances: instances=%s" % instances)
        """
        LOG.info(">>>>>>>>>>>>>>>  build_instances: context=%s" % context)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: image=%s" % image)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: filter_properties=%s" % filter_properties)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: admin_password=%s" % admin_password)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: injected_files=%s" % injected_files)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: requested_networks=%s" % requested_networks)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: security_groups=%s" % security_groups)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: block_device_mapping=%s" % block_device_mapping)
        LOG.info(">>>>>>>>>>>>>>>  build_instances: legacy_bdm=%s" % legacy_bdm)
        """
        """
    ...
    def build_instance(self, context, instance, image, filter_properties, admin_password, injected_files, requested_networks, security_groups, block_device_mapping=None, legacy_bdm=True):            
        try:
            ...
            #LOG.info(">>>>>>>>>>>>>>>>> build_instance instance %s" % instance)
            #LOG.info(">>>>>>>>>>>>>>>>> build_instance filter_properties %s" % filter_properties)
            #LOG.info(">>>>>>>>>>>>>>>>> build_instance context %s" % context)
            ...
            if not self.client.can_send_version('1.7'):
                version = '1.5'
                kw.update({'block_device_mapping': block_device_mapping, 'legacy_bdm': legacy_bdm})
    ...
    def object_class_action(self, context, objname, objmethod, objver, args, kwargs):
        ...
        client = rpc.getClient(target=messaging.Target(topic=topic + "_synergy", version="2.0"), version_cap="2.0")
...
class QueueWorker(threading.Thread):
    ...
    def __init__(self, name, queue, quota_manager, novaComputeAPI, novaConductorAPI, novaConductorComputeAPI):
    ...
    def run(self):
        ...
        while not self.exit and not self.queue.isClosed():
            ...
            try:
                ...
                try:
                    vm_instance = self.novaConductorAPI.instance_get_by_uuid(context, instance_uuid=instance_uuid)
                except Exception as ex:
                    LOG.warn("QueueWorker %s: instance %s not found!" % (self.name, instance_uuid))
                    self.queue.deleteItem(queue_item)
                    continue
                    
                if vm_instance["vm_state"] != "building" or vm_instance["task_state"] != "scheduling":
                    self.queue.deleteItem(queue_item)
                    continue
                ...
                if quota.allocate(instance_uuid, vcpus, memory, blocking):
                    try:
                        self.novaConductorComputeAPI.build_instance(context=context, instance=request["instance"], image=request["image"],
                                                           filter_properties=request["filter_properties"], admin_password=request["admin_password"],
                                                           injected_files=request["injected_files"], requested_networks=request["requested_networks"],
                                                           security_groups=request["security_groups"], block_device_mapping=request["block_device_mapping"],
                                                           legacy_bdm=request["legacy_bdm"])

                        LOG.info("QueueWorker '%s' build_instance OK: instace_id=%s" % (self.name, instance_uuid))
                    except Exception as ex:
                        LOG.error("QueueWorker '%s' build_instance ERROR: instace_id=%s reason=%s" % (self.name, instance_uuid, ex))
                        quota.release(instance_uuid, vcpus, memory)

                self.queue.deleteItem(queue_item)

                """
                while not done and queue_item.getRetryCount() < 3:
                    allocated = quota.allocate(instance_uuid, vcpus, memory, blocking)
                    if allocated:
                        try:                            
                            self.novaConductorComputeAPI.build_instance(context=context, instance=request["instance"], image=request["image"],
                                                               filter_properties=request["filter_properties"], admin_password=request["admin_password"],
                            ...
                            LOG.error("QueueWorker '%s' build_instance error: %s" % (self.name, ex))
                            
                            queue_item.incRetryCount()
                            self.queue.updateItem(queue_item)
                            time.sleep(5)
                ...
                if not done and queue_item.getRetryCount() >= 3:
                    self.queue.deleteItem(queue_item)
                """
            except Exception as ex:
                LOG.error("QueueWorker '%s': %s" % (self.name, ex))
        ...
        #LOG.info(">>>> QueueWorker '%s' queue.isClosed %s exit=%s" % (self.name, self.queue.isClosed(), self.exit))
...
class FairShareManager(manager.Manager):
    ...
    def __init__(self):
        super(FairShareManager, self).__init__(name="FairShareManager", autostart=True, rate=60)
        ...
        self.config_opts = [
            ...
            cfg.ListOpt("dynamic_quota", default = [], help = "the projects list with dynamic quota"),
            cfg.ListOpt("extended_quota", default = [], help = "the projects list with extended quota")
        ]
    ...
    def execute(self, cmd):
        ...
        def makeProjectInfo(project_id):
            ...
            if self.usage_table.has_key(project_id):
                project = self.usage_table.get(project_id)
                project_info = { "name": project["name"], "share": project["share"], "actual_vcpus_usage": project["actual_vcpus_usage"], "actual_memory_usage": project["actual_memory_usage"], "users": {}}
                ...
                for user_id, user in project.get("users", {}).items():                            
                     ...
                     project_info["users"][user_id] = { "name": user["name"], "share": user["share"], "relative_share": relative_share, "absolute_share": user["normalized_share"], "priority": priority, "normalized_vcpus_usage": user["normalized_vcpus_usage"], "normalized_memory_usage": user["normalized_memory_usage"], "relative_vcpus_usage": relative_vcpus_usage, "relative_memory_usage": relative_memory_usage }

                     #project_info["users"][user_id] = { "name": user["name"], "share": user["share"], "relative_share": relative_share, "absolute_share": user["normalized_share"], "priority": priority, "normalized_vcpus_usage": user["effective_vcpus_usage"], "normalized_memory_usage": user["effective_memory_usage"], "relative_vcpus_usage": user["effective_vcpus_usage_relative"], "relative_memory_usage": user["effective_memory_usage_relative"] }
        ...
        if cmd.getName() == "get_priority":                  
            ...
            if project_ids:            
                ...
                if len(project_list) == 1 and len(cmd.getResults()) == 0:                
                    raise Exception("project id='%s' not found!" % project_list[0])
                ...
                if len(project_list) == 1 and len(cmd.getResults()) == 0:                
                    raise Exception("project name='%s' not found!" % project_list[0])
    ...
    def setup(self):
        ...
        try:
            self.keystone_client = keystone_v3.KeystoneClient(auth_url=self.keystone_auth_url, username=self.keystone_admin_user, password=self.keystone_admin_password, project_name=self.keystone_admin_project_name)
            ...
            nova_cfg_file = cfg.find_config_files(project="nova", extension='.conf')
            ...
            if len(nova_cfg_file) > 0 and os.path.isfile(nova_cfg_file[0]):
                ...
                try:
                    ...
                    raise Exception("No option 'my_ip' found in %s: using default '%s'" % (nova_cfg_file, server))
                
                try:
                    conductor_topic = CONFIG.get("DEFAULT", "conductor_topic")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'conductor_topic' found in %s: using default '%s'" % (nova_cfg_file, conductor_topic)) 
                    
                try:
                    compute_topic = CONFIG.get("DEFAULT", "compute_topic")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'compute_topic' found in %s: using default '%s'" % (nova_cfg_file, compute_topic)) 
                    
                try:
                    scheduler_topic = CONFIG.get("DEFAULT", "scheduler_topic")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'scheduler_topic' found in %s: using default '%s'" % (nova_cfg_file, scheduler_topic))                    
            else: 
            ...
            self.novaConductorComputeAPI = NovaConductorComputeAPI(conductor_topic, self.processRequest, self.keystone_client)
                                    
            self.rpcserver = rpc.getServer(target=messaging.Target(topic=conductor_topic, server=server), endpoints=[self.novaBaseAPI, self.novaConductorAPI, self.novaConductorComputeAPI])
            self.rpcserver.start()
            
            self.novaComputeAPI = NovaComputeAPI(compute_topic)
            
            self.rpcserver = rpc.getServer(target=messaging.Target(topic=compute_topic, server=server), endpoints=[self.novaComputeAPI])
            #self.rpcserver.start()
            ...
            for project in self.extended_quota:
                ...
                if len(parsed_limit) > 1:
                    if not parsed_limit[-1].isdigit():
                        raise Exception("wrong runtime limit '%s' found in '%s'!" % (parsed_limit[-1], project))

                    if len(parsed_limit) == 2:
                        project_name = parsed_limit[0]
                        runtime_limit = int(parsed_limit[1])
                    else:
                        raise Exception("wrong runtime limit definition: '%s'" % project)                        
                else:
                ...
                for k_project in k_projects:
                    if k_project["name"] == project_name:
                        ...
                        self.projects[k_project["name"]] = { "name": k_project["name"], "id": k_project["id"], "type": "extended", "share": self.default_project_share, "runtime_limit": runtime_limit, "users": {}}                        
                        self.extended_projects.append(project_name)
                        
                        quota = self.quota_manager.getQuota(project_id=k_project["id"])                
                        ...
                        queue_worker = QueueWorker(queue.getName(), queue, self.quota_manager, self.novaComputeAPI, self.novaConductorAPI, self.novaConductorComputeAPI)
                        queue_worker.start()

                        self.queue_workers.append(queue_worker)
                if not found:
                    LOG.warn("project '%s' not found in Keystone: ignored!" % (project_name))
            ...
            for project in self.dynamic_quota:
                ...
                for k_project in k_projects:
                    if k_project["name"] == project_name:
                        ...
                        self.projects[k_project["name"]] = { "name": k_project["name"], "id": k_project["id"], "type": "dynamic", "share": self.default_project_share, "runtime_limit": runtime_limit, "users": {}}
            ...
            for k_project in k_projects:
                if not k_project["name"] in self.dynamic_projects and not k_project["name"] in self.extended_projects:
                    try:
                        queue = self.queue_manager.createQueue(name=k_project["name"])
                    except Exception as ex:
                        queue = self.queue_manager.getQueue(name=k_project["name"])
            ...
            for share in self.shares:
                ...
                if len(parsed_share) > 1:
                    if not parsed_share[-1].isdigit():
                        raise Exception("wrong share value '%s' found in '%s'!" % (parsed_share[-1], share))
                ...
                if not self.projects.has_key(project_name):
                    LOG.warn("project '%s' not found in the projects list %s: ignored!" % (project_name, self.projects))
                    continue
            ...
            dynamic_queue_worker = QueueWorker(dynamic_queue.getName(), dynamic_queue, self.quota_manager, self.novaComputeAPI, self.novaConductorAPI, self.novaConductorComputeAPI)
    ...
    def processRequest(self, request):
        ...
        try:
            ...
            if self.usage_table.has_key(project_id):
                ...
                try:
                    ...
                    if filter_properties.has_key("retry"):
                        ...
                        if num_attempts > 0:
                            ...
                            LOG.info("released resource uuid %s num_attempts %s" % (uuid, num_attempts))
                except Exception as ex:
                    LOG.error(ex)

                if priority == 0:
                    priority = self.calculatePriority(user_id, project_id, timestamp)

                if project_name in self.dynamic_projects:
                    queue = self.queue_manager.getQueue("dynamic")
                    queue.insertItem(user_id, project_id, priority=priority, data=request)

                    LOG.info("new request: instance_id=%s user_id=%s project_id=%s priority=%s queue=%s type=dynamic" % (uuid, user_id, project_id, priority, queue.getName()))

                elif project_name in self.extended_projects:
                    queue = self.queue_manager.getQueue(project_name)
                    queue.insertItem(user_id, project_id, priority=priority, data=request)

                    LOG.info("new request: instance_id=%s user_id=%s project_id=%s priority=%s queue=%s type=extended" % (uuid, user_id, project_id, priority, queue.getName()))
                ...
                LOG.info("new request: instance_id=%s user_id=%s project_id=%s queue=%s type=static" % (uuid, user_id, project_id, priority, queue.getName()))
    ...
    def calculatePriority(self, user_id, project_id, timestamp=None, retry_count=0):
        if not self.usage_table.has_key(project_id) or not self.usage_table[project_id]["users"].has_key(user_id):
            raise Exception("user=%s project=%s not found!" % (user_id, project_id))
        ...
        priority = float(self.age_weight) * minutes + float(self.vcpus_weight) * fair_share_vcpus + float(self.memory_weight) * fair_share_memory - float(self.age_weight) * retry_count
    ...
    def calculateFairShares(self, projects):
        ...
        for user in users:
            ...
            for project in user_projects:
                ...
                if not usage_table.has_key(project_id):
                    usage_table[project_id] = { "name":project_name, "share":float(self.projects[project_name]["share"]), "users":{} }

                users = usage_table[project_id]["users"]

                if not users.has_key(user_id):
                    users[user_id] = { "name":user_name, "usage_records":[], "normalized_vcpus_usage":float(0), "normalized_memory_usage":float(0) }
        
        for x in xrange(self.num_of_periods):
            self.__getUsage(usage_table=usage_table, fromDate=str(now - timedelta(days=(x * self.period_length))), period_length=self.period_length)
        ...
        for project_id, project in usage_table.items():
            ...
            for user_id, user in users.items():
                ...
                if self.projects.has_key(project_name) and self.projects[project_name]["users"].has_key(user_name):
                    user_share = self.projects[project_name]["users"][user_name]
        ...
        for project_id, project in usage_table.items():
            ...
            for user_id, user in users.items():
                # for each user the normalized share is calculated (0 <= user_normalized_share <= 1)
                user_share = float(user["share"])

                if project_share > 0 and sibling_share > 0 and total_share > 0:
                    user_normalized_share = (user_share / sibling_share) * (project_share / total_share)
                else:
                    user_normalized_share = user_share
                    
                user["normalized_share"] = user_normalized_share
                
                # calculate the normalized_vcpus_usage, normalized_memory_usage, historical_vcpus_usage and the historical_memory_usage
                index = 0
                ...
                for usage_record in user["usage_records"]:
                    ...
                    historical_vcpus_usage += decay * usage_record["vcpus_usage"]
                    historical_memory_usage += decay * usage_record["memory_usage"]
                    LOG.info(">>>>>>>>>>>>>> name=%s historical_vcpus_usage=%s" % (user["name"] ,historical_vcpus_usage))

                    index += 1
                    
                if total_historical_vcpus_usage > 0:
                    user["normalized_vcpus_usage"] = historical_vcpus_usage / total_historical_vcpus_usage
                else:
                    user["normalized_vcpus_usage"] = historical_vcpus_usage
 
                if total_historical_memory_usage > 0:
                    user["normalized_memory_usage"] = historical_memory_usage / total_historical_memory_usage
                else:
            ...
            LOG.info(">>>>>>>>>>>>>> total_actual_vcpus_usage=%s" % total_actual_vcpus_usage) 
        ...
        for project in usage_table.values():
            ...
            for user in users.values():
                ...
                if share == 0:
                    ...
                    effective_vcpus_usage = normalized_vcpus_usage + ((effective_project_vcpus_usage - normalized_vcpus_usage) * share / sibling_share)
                    effective_memory_usage = normalized_memory_usage + ((effective_project_memory_usage - normalized_memory_usage) * share / sibling_share)

                    user["effective_vcpus_usage"] = effective_vcpus_usage
                    user["effective_memory_usage"] = effective_memory_usage
                    LOG.info(">>>>>>>>>>>>>> actual_vcpus_usage=%s" % actual_vcpus_usage)
                    if actual_vcpus_usage > 0:
                        user["effective_vcpus_usage_relative"] = normalized_vcpus_usage / actual_vcpus_usage
                    else:
                        user["effective_vcpus_usage_relative"] = 0

                    if actual_memory_usage > 0:
                        user["effective_memory_usage_relative"] = normalized_memory_usage / actual_memory_usage
                    else:
                    ...
                    if normalized_share == 0:
                        ...
                        user["fair_share_vcpus"] = 2 ** (-effective_vcpus_usage / normalized_share)
                        user["fair_share_memory"] = 2 ** (-effective_memory_usage / normalized_share)
                    #LOG.info(user)
    ...
    def __getUsage(self, usage_table, fromDate, period_length):
        ...
        conn = MySQLdb.connect(self.mysql_host, self.mysql_user, self.mysql_password)
        cursor = conn.cursor()
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if not project["users"].has_key(user_id):
                    LOG.warn("user not found: %s" % user_id)
                    continue

                usage_record = { "memory_usage":float(row[2]), "vcpus_usage":float(row[3]) }
        ...
        for project_id, project in self.usage_table.items():
            ...
            for user_id, user in project["users"].items():
                ...
                try:
                    ...
                    cursor.execute("select count(*) from nova.instances where terminated_at is null and launched_at is not null and deleted_at is null and user_id='"+user_id+"' and project_id='"+project_id+"'")
                    row = cursor.fetchone()
                    vmInstances = row[0]
                except Exception as ex:
                    LOG.error(ex)
                ...
                msg += "{0:10s}| {1:13s}| {2:11s}| {3:14s}| {4:19s}| {5:20s}| {6:19s}| {7:19s}| {8:9}| {9:6}|\n".format(user["name"], project["name"], str(user["share"]) + "%", str(project["share"]) + "%", str(user["fair_share_vcpus"]), str(user["fair_share_memory"]), "{0:.1f}%".format(user["normalized_vcpus_usage"]*100), "{0:.1f}%".format(user["effective_vcpus_usage"]*100), str(int(user["fair_share_vcpus"]*self.vcpus_weight + user["fair_share_memory"]*self.memory_weight)), str(vmInstances))
