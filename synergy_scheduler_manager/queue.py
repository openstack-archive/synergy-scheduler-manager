...
class QueueItem():
    ...
    def __init__(self, id, user_id, project_id, priority, retry_count, creation_time, last_update, data=None):
        self.id = id
    ...
    def buildFromDB(self):
        try:
            ...
            for row in cursor.fetchall():
                queue_item = QueueItem(row[0], row[1], row[2], row[3], row[4], row[5], row[6])
    ...
    def insertItem(self, userId, projectId, priority, data):
        with self.condition:
            ...
            try:
                ...
                cursor = dbConnection.cursor()
                cursor.execute(QUERY, [userId, projectId, priority, json.dumps(data)])
            ...
            queue_item = QueueItem(idRecord, userId, projectId, priority, 0, now, now)
        ...
        with self.condition:
            ...
            if (not self.is_closed and queue_item is not None):                
                try:
                    ...
                    item = QueueItem(queue_item.getId(), row[0], row[1], row[2], row[3], row[4], row[5], json.loads(row[6]))
                except Exception as ex:
                    LOG.error(ex)
                    raise ex
                finally:
    ...
    def updateItem(self, queue_item):        
        ...
        with self.condition:        
            try:                
                ...
                cursor = dbConnection.cursor()
                cursor.execute(QUERY, [queue_item.getPriority(), queue_item.getRetryCount(), queue_item.getLastUpdate(), queue_item.getId()])
        ...
        with self.condition:
            ...
            while not self.pqueue.empty():
                ...
                try:
                    ...
                    cursor = dbConnection.cursor()
                    cursor.execute(QUERY, [queue_item.getPriority(), queue_item.getLastUpdate(), queue_item.getId()])
...
class QueueManager(manager.Manager):
    ...
    def __init__(self):
        super(QueueManager, self).__init__(name="QueueManager", autostart=True, rate=60)
        ...
        if cmd.getName() == "get_queue":
            ...
            if queue_name:
                ...
                for item in item_list:
                    if self.queue_list.has_key(item):
    ...
    def setup(self):
        ...
        try:
            ...
            self.pool = None
            self.pool = PooledDB(mysql.connector, poolSize, database = database, user = user, passwd = passwd, host = host)
        except Exception as ex:
            LOG.error(ex)
            raise ex
    ...
    def createQueue(self, name):
        if not self.queue_list.has_key(name):
