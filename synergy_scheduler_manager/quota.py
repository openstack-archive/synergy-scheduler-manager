...
class Quota(object):
    ...
    def __init__(self, name, project_id, quota_type, nova_client, pool):
        LOG.info("new Quota project_id=%s quota_type=%s" % (project_id, quota_type))
        self.name = name
        ...
        self.instances = { "in_use":0, "hard_limit":0, "ids":[], "pending":[], "processed":0, "canc_active":0, "canc_error":0, "canc_building":0 }
        self.condition = None
        self.exit = False
    ...
    def __storeQuotaDB(self):
        try:        
            ...
            dbConnection = self.pool.connection()
            cursor = dbConnection.cursor()
            cursor.execute(QUERY, { "project_id": self.project_id, "name": self.name, "type": 0, "cores": self.cores["hard_limit"], "ram": self.ram["hard_limit"], "instances": self.instances["hard_limit"] })
        ...
        if self.quota_type == "dynamic":
            ...
            if self.runtime_limit > 0:
                try:
                    ...
                    instance_id_list = "'%s'" % "','".join(self.instances["ids"]) 
                    ...
                    query = query % { "project_id": self.project_id, "instances": instance_id_list, "expiration": self.runtime_limit }
                    #LOG.info(query)
                    ...
                    for row in cursor.fetchall():
                        ...
                        try:
                            LOG.info("deleting the expired instance '%s' from project=%s" % (instance_id, self.project_id))
            ...
            if project_quota["cores"] == -1 and project_quota["ram"] == -1:            
                if cores == 0 and ram == 0:
                    project_quota_defaults = self.nova_client.getQuota(self.project_id, True)
            ...
            if self.cores["hard_limit"] != cores or self.ram["hard_limit"] != ram or self.instances["hard_limit"] != instances:        
                with self.condition:                
                    self.cores["hard_limit"] = cores
                    self.ram["hard_limit"] = ram
                    self.instances["hard_limit"] = instances
    ...
    def setType(self, quota_type):
        if quota_type != "static" and quota_type != "extended" and quota_type != "dynamic":
            raise Exception("wrong quota type %s, use 'static', 'extended' or 'dynamic'" % (self.quota_type))
        ...
        try:
            if self.quota_type == "static" or self.quota_type == "extended":
                quota_set = { "cores": self.cores["hard_limit"], "ram": self.ram["hard_limit"], "instances": self.instances["hard_limit"] }
                project_quota = self.nova_client.updateQuota(self.project_id, quota_set)
            else:
    ...
    def allocate(self, instance_id, cores, ram, blocking=True):
        LOG.info("allocate instance_id=%s cores=% ram=%s" % (instance_id, cores, ram))
        found = False
        ...
        with self.condition:
            ...
            while (not found and instance_id in self.instances["pending"] and not self.exit): 
                if self.isDynamicType(): 
                    LOG.info("allocate name=%s type=%s instance_id=%s cores=%s ram=%s dynamic [vcpu in_use %s of %s; ram in use %s of %s]" % (self.getName(), self.getType(), instance_id, cores, ram, Quota.cores["dynamic_in_use"], Quota.cores["total"] - Quota.cores["static_assigned"], Quota.ram["dynamic_in_use"], Quota.ram["total"] - Quota.ram["static_assigned"]))   

                    if (Quota.cores["total"] - Quota.cores["static_assigned"] - Quota.cores["dynamic_in_use"] >= cores) and (Quota.ram["total"] - Quota.ram["static_assigned"] - Quota.ram["dynamic_in_use"] >= ram):
                        ...
                        LOG.info("allocated name=%s type=%s instance_id=%s cores=%s ram=%s dynamic [vcpu in_use %s of %s; ram in use %s of %s]" % (self.getName(), self.getType(), instance_id, cores, ram, Quota.cores["dynamic_in_use"], Quota.cores["total"] - Quota.cores["static_assigned"], Quota.ram["dynamic_in_use"], Quota.ram["total"] - Quota.ram["static_assigned"]))
                    ...
                    LOG.info("allocate name=%s type=%s instance_id=%s cores=%s of %s ram=%s of %s" % (self.getName(), self.getType(), instance_id, cores, ram, self.getCores(), self.getRam()))
                    ...
                    LOG.info("allocated name=%s type=%s instance_id=%s cores=%s of %s ram=%s of %s" % (self.getName(), self.getType(), instance_id, cores, ram, self.getCores(), self.getRam()))
                    #self.condition.notifyAll()
    ...
    def release(self, instance_id, cores, ram):
        LOG.info("release: instance_id=%s cores=%s ram=%s before release: cores=%s ram=%s d_cores=%s d_ram=%s" % (instance_id, cores, ram, self.cores["in_use"], self.ram["in_use"], Quota.cores["dynamic_in_use"], Quota.ram["dynamic_in_use"]))
        ...
        with self.condition:
            if instance_id in self.instances["pending"]:
                ...
                LOG.info("release: instance_id=%s cores=%s ram=%s after release: cores=%s ram=%s d_cores=%s d_ram=%s" % (instance_id, cores, ram, self.cores["in_use"], self.ram["in_use"], Quota.cores["dynamic_in_use"], Quota.ram["dynamic_in_use"]))
            else:
    ...
    def toDict(self):
        ...
        if self.isDynamicType():
            quota["ram"] = { "in_use_p": self.ram["in_use"], "in_use": Quota.ram["dynamic_in_use"], "hard_limit": Quota.ram["total"] - Quota.ram["static_assigned"] }
            quota["cores"] = { "in_use_p": self.cores["in_use"], "in_use": Quota.cores["dynamic_in_use"], "hard_limit": Quota.cores["total"] - Quota.cores["static_assigned"] }
        else:        
            quota["ram"] = { "in_use": self.ram["in_use"], "hard_limit": self.ram["hard_limit"] }
            quota["cores"] = { "in_use": self.cores["in_use"], "hard_limit": self.cores["hard_limit"] }
...
class QuotaManager(manager.Manager):
    ...
    def __init__(self):
        super(QuotaManager, self).__init__(name="QuotaManager", autostart=True, rate=60)
        #eventlet.monkey_patch()
                 
        self.config_opts = [
            cfg.IntOpt("quota_instances", default=10, help="Number of instances allowed per project"),
            cfg.IntOpt("quota_cores", default=20, help="Number of instance cores allowed per project"),
            cfg.IntOpt("quota_ram", default=50 * 1024, help="Megabytes of instance RAM allowed per project"),
            cfg.ListOpt("dynamic_quotas", default = [], help = "the projects list with dynamic quota")
        ]
    ...
    def setup(self):
        ...
        try:
            ...
            self.pool = None
            self.pool = PooledDB(mysql.connector, poolSize, database=mysql_db, user=mysql_user, passwd=mysql_passwd, host=mysql_host)
        except Exception as ex:
            LOG.error(ex)
            raise ex
        ...
        try:        
            ...
            if len(nova_cfg_file) > 0 and os.path.isfile(nova_cfg_file[0]):
                try:
                    self.cpu_allocation_ratio = CONFIG.getfloat("DEFAULT", "cpu_allocation_ratio")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'cpu_allocation_ratio' found in %s: using default '%s'" % (nova_cfg_file, self.cpu_allocation_ratio))
                    
                try:
                    self.ram_allocation_ratio = CONFIG.getfloat("DEFAULT", "ram_allocation_ratio")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'ram_allocation_ratio' found in %s: using default '%s'" % (nova_cfg_file, self.ram_allocation_ratio))
                    
                try:
                    self.disk_allocation_ratio = CONFIG.getfloat("DEFAULT", "disk_allocation_ratio")
                except Exception as ConfigParser.NoOptionError:
                    LOG.info("No option 'disk_allocation_ratio' found in %s: using default '%s'" % (nova_cfg_file, self.disk_allocation_ratio))
            else:
            ...
            self.keystone_client = keystone_v3.KeystoneClient(auth_url=keystone_auth_url, username=keystone_admin_user, password=keystone_admin_password, project_name=keystone_admin_project_name)
            ...
            self.target = [ messaging.Target(topic='notifications', version="3.0") ]
        ...
        if cmd.getName() == "get_quota":
            ...
            if project_ids:
                ...
                for project_id in item_list:
                    if self.quotas.has_key(project_id):
                ...
                if len(item_list) == 1 and len(cmd.getResults()) == 0:                
                    raise Exception("quota for project id='%s' not found!" % item_list[0])
                ...
                if len(item_list) == 1 and len(cmd.getResults()) == 0:                
                    raise Exception("quota for project name='%s' not found!" % item_list[0])
    ...
    def task(self):
        with self.condition:
            try:
                ...
                for project_id, quota in self.quotas.items():
                    LOG.info("quota name='%s' type='%s' project_id='%s' cores=%s ram=%s instances=%s" % (quota.getName(), quota.getType(), project_id, quota.getCores(), quota.getRam(), quota.getInstances()))
    ...
    #mysql> select sum(vcpus), sum(memory_mb), sum(local_gb) from compute_nodes;
    ...
    def info(self, ctxt, publisher_id, event_type, payload, metadata):
        ...
        if (event_type == "compute.instance.delete.end" and (state == "deleted" or state == "error" or state == "building")) or (event_type == "compute.instance.update" and (state == "deleted" or state == "error")) or (event_type == "scheduler.run_instance" and state == "error"):
            ram = 0
            cores = 0
            project_id = 0
            instance_info = None
            ...
            LOG.info("Notification INFO (type=%s state=%s): cores=%s ram=%s project_id=%s instance_id=%s" % (event_type, state_description, cores, ram, project_id, instance_id))
    ...
    def warn(self, ctxt, publisher_id, event_type, payload, metadata):
        ...
        #state = payload["state"]
        instance_id = payload["instance_id"]
        LOG.info("Notification WARN: event_type=%s state=%s instance_id=%s" % (event_type, state, instance_id))
        

    def error(self, ctxt, publisher_id, event_type, payload, metadata):
        LOG.info("Notification ERROR: event_type=%s payload=%s metadata=%s" % (event_type, payload, metadata))
        ...
        if event_type == "scheduler.run_instance" and state == "error":
            ...
            project_id = payload["request_spec"]["instance_properties"]["project_id"]
            instance_id = payload["instance_id"]

            LOG.info("Notification ERROR (%s): cores=%s ram=%s project_id=%s instance_id=%s" % (event_type, cores, ram, project_id, instance_id))
    ...
    def getQuota(self, project_id):
        if not self.quotas.has_key(project_id):
        ...
        try:
            ...
            for hypervisor in hypervisors:
                if hypervisor["status"] == "enabled" and hypervisor["state"] == "up":                
                    hypervisor_details = self.nova_client.getHypervisor(hypervisor["id"])
            ...
            for project in kprojects:            
                ...
                if self.quotas.has_key(project_id):
                    ...
                    quota = Quota(project_name, project_id, quota_type, self.nova_client, self.pool)                    
            ...
            if total_cores < total_cores_limit:
                LOG.warn("dynamic quota disabled: the total hard limit for cores ('%s') is greater than the total amount of cores allowed ('%s')" % (total_cores_limit, total_cores))
                total_dynamic_cores_limit = 0
            else:            
                total_dynamic_cores_limit = total_cores - total_cores_limit

            if total_ram < total_ram_limit:         
                LOG.warn("dynamic quota disabled: the total hard limit for ram ('%s') is greater than the total amount of ram allowed ('%s')" % (total_ram_limit, total_ram))
                total_dynamic_ram_limit = 0
            else:
            ...
            LOG.info(">>>>>>> total_dynamic_cores_limit %s" % total_dynamic_cores_limit)
            LOG.info(">>>>>>> total_dynamic_ram_limit %s" % total_dynamic_ram_limit)
