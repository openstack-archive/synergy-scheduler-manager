...
class Notifications(object):
    ...
    def info(self, ctxt, publisher_id, event_type, payload, metadata):
        ...
        if ((event_type == "compute.instance.delete.end" and
            ...
            # server.setCreatedAt(instance_info["created_at"])
...
class Worker(Thread):
    ...
    def run(self):
        ...
        queue_items = []
        lastReleaseTime = SharedQuota.getLastReleaseTime()

        while not self.exit and not self.queue.isClosed():
            if lastReleaseTime < SharedQuota.getLastReleaseTime():
                lastReleaseTime = SharedQuota.getLastReleaseTime()
