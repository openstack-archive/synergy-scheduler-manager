...
class SchedulerManager(Manager):
    ...
    def processRequest(self, request):
        try:
            filter_prop = request["filter_properties"]
            instance = request["instance"]
            user_id = instance["nova_object.data"]["user_id"]
            prj_id = instance["nova_object.data"]["project_id"]
            uuid = instance["nova_object.data"]["uuid"]
            ...
            if prj_id in self.projects:
                ...
                try:
                    if "retry" in filter_prop:
                        num_attempts = filter_prop["retry"]["num_attempts"]
...
def main():
    ...
    try:
        ...
        from nova_manager import NovaManager
        ...
        print("configuring KeystoneManager")
        keystone_manager = KeystoneManager()
        CONF.register_opts(keystone_manager.config_opts,
                           group="KeystoneManager")

        print("configuring NovaManager")
        nova_manager = NovaManager()
        CONF.register_opts(nova_manager.config_opts, group="NovaManager")

        print("configuring QueueManager")
        queue_manager = QueueManager()
        CONF.register_opts(queue_manager.config_opts, group="QueueManager")

        print("configuring QuotaManager")
        quota_manager = QuotaManager()
        CONF.register_opts(quota_manager.config_opts, group="QuotaManager")

        print("configuring FairShareManager")
        fairshare_manager = FairShareManager()
        CONF.register_opts(fairshare_manager.config_opts,
                           group="FairShareManager")

        print("configuring SchedulerManager")
        scheduler_manager = SchedulerManager()
        CONF.register_opts(scheduler_manager.config_opts,
                           group="SchedulerManager")

        managers = {}
        managers["KeystoneManager"] = keystone_manager
        managers["NovaManager"] = nova_manager
        managers["QueueManager"] = queue_manager
        managers["QuotaManager"] = quota_manager
        managers["FairShareManager"] = fairshare_manager
        managers["SchedulerManager"] = scheduler_manager

        keystone_manager.managers = managers
        nova_manager.managers = managers
        queue_manager.managers = managers
        quota_manager.managers = managers
        fairshare_manager.managers = managers
        scheduler_manager.managers = managers

        keystone_manager.setup()
        nova_manager.setup()
        queue_manager.setup()
        quota_manager.setup()
        fairshare_manager.setup()
        scheduler_manager.setup()

        fairshare_manager.calculateFairShare()

        print("test SchedulerManager end")
    except Exception as ex:
        print("unrecoverable error: %s" % ex)
