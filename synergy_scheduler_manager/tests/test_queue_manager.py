...
class TestQueue(base.TestCase):
    ...
    def test_insert_item(self):
        ...
        q = Queue(name="test", db_engine=db_engine_mock)
        ...
        self.assertIn(insert_call, db_engine_mock.mock_calls)
    ...
    def test_get_size(self):
        # Create a queue with a mocked db engine
        db_engine_mock = create_autospec(Engine)
        q = Queue(name="test", db_engine=db_engine_mock)

        # Mock the return value of select count
        select_mock = db_engine_mock.connect().execute(
            'select count(*) from `test`').fetchone
        select_mock.return_value = [3]

        # Check that getSize() uses the correct method and SQL statement
        self.assertEqual(3, q.getSize())
